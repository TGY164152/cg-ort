...
import com.ww.ort.service.DfOrtExperStandConfigService;
 ...
 */
@Controller
@RequestMapping("/dfOrtExperStandConfig")
@Api(tags = "ORT强度标准值配置")
@ResponseBody
@CrossOrigin
public class DfOrtExperStandConfigController {
    ...
    public Result listBySearch(
        ...
        IPage<DfOrtExperStandConfig> pages = new Page<>(page,limit);

        QueryWrapper<DfOrtExperStandConfig> qw = new QueryWrapper<>();
        ...
        IPage<DfOrtExperStandConfig> list = dfOrtExperStandConfigService.page(pages,qw);
    ...
    public Result saveOrUpdate(@RequestBody DfOrtExperStandConfig data){
        if (data.getId()!=null){
            if (dfOrtExperStandConfigService.updateById(data)){
                return new Result(200,"修改成功");
            }
            return new Result(500,"修改失败");
        }else {
            if (dfOrtExperStandConfigService.save(data)){
    ...
    public Result delete(Integer id){
        if (dfOrtExperStandConfigService.removeById(id)){
