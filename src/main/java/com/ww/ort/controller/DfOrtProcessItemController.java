...
@Controller
@RequestMapping("/dfOrtProcessItem")
@Api(tags = "ORT工序-测试项")
@ResponseBody
@CrossOrigin
public class DfOrtProcessItemController {

    @Autowired
    private DfOrtProcessItemService dfOrtProcessItemService;
    ...
    public Result listBySearch(
        ...
        IPage<DfOrtProcessItem> pages = new Page<>(page,limit);

        QueryWrapper<DfOrtProcessItem> qw = new QueryWrapper<>();
        ...
        IPage<DfOrtProcessItem> list = dfOrtProcessItemService.page(pages,qw);
    ...
    public Result saveOrUpdate(@RequestBody DfOrtProcessItem data){
        if (data.getId()!=null){
            if (dfOrtProcessItemService.updateById(data)){
                return new Result(200,"修改成功");
            }
            return new Result(500,"修改失败");
        }else {
            if (dfOrtProcessItemService.save(data)){
    ...
    public Result delete(Integer id){
        if (dfOrtProcessItemService.removeById(id)){
    ...
    public Result listAll(
        ...
        List<DfOrtProcessItem> list = dfOrtProcessItemService.list(qw);
